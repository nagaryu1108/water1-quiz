(function(){
  /* Stable entry point: preserve IDs/history while layering quality patches. */
  if(document.readyState==='loading'){
    document.write('<script src="./qbank_core_patch_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_detail_patch_v6.js?v=124"><\/script>');
    document.write('<script src="./qbank_enrichment_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_detail_round1_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_detail_complete_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch1_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch2_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch3_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch4_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch4_fix_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch5_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch6_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch6_fix_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch7_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch8_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch9_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch10_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch11_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_gap_batch12_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_detail_quality_fix_v23.js?v=124"><\/script>');
    document.write('<script src="./qbank_undefined_fix_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_exam_quality_patch_v34.js?v=125"><\/script>');
    document.write('<script src="./qbank_visual_fix_v35.js?v=126"><\/script>');
    document.write('<script src="./qbank_unit_typography_v1.js?v=127"><\/script>');
    document.write('<script src="./qbank_frequency_scheduler_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_chem_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_quality_audit_v7.js?v=124"><\/script>');
    document.write('<script src="./qbank_regression_audit_v14.js?v=124"><\/script>');
    document.write('<script src="./qbank_canonical_gap_audit_v21.js?v=124"><\/script>');
    document.write('<script src="./qbank_content_restructure_v39.js?v=130"><\/script>');
    document.write('<script src="./qbank_distractor_plausibility_v40.js?v=131"><\/script>');
    document.write('<script src="./qbank_semantic_rewrite_gw_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_rewrite_th_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_rewrite_l_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_strengthen_g_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_strengthen_w_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_strengthen_t_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_strengthen_h_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_strengthen_l_v42.js?v=132"><\/script>');
    document.write('<script src="./qbank_semantic_polish_v43.js?v=133"><\/script>');
    document.write('<script src="./qbank_semantic_polish_v44.js?v=134"><\/script>');
    document.write('<script src="./qbank_semantic_polish_v45.js?v=135"><\/script>');
    document.write('<script src="./ui_polish_v46.js?v=136"><\/script>');
  }
})();