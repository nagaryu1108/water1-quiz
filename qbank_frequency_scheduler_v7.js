...
/* Evidence-count scheduler v9. Counts are publicly confirmed LOWER BOUNDS, not a claimed full census. */
var PROFILES=[
...
{name:'シアン',yearsMin:6,questionsMin:10,weight:2.95,re:/シアン|CN|アルカリ塩素|紺青/},
{name:'水質法令・基準・地下水',yearsMin:6,questionsMin:21,weight:2.90,re:/水質汚濁防止法|水濁法|環境基準|排水基準|地下水|常時監視|総量規制|特定地下浸透水|指定施設|指定物質|有害物質貯蔵指定施設/},
{name:'冷却水・再利用',yearsMin:6,questionsMin:7,weight:2.75,re:/冷却水|濃縮倍数|ブロー|カスケード|再利用|循環冷却/},
...
{name:'水域モデル・富栄養化',yearsMin:5,questionsMin:10,weight:2.45,re:/生態系モデル|L-Q|富栄養化|内部負荷|海域DO|貧酸素|植物プランクトン|POC|クロロフィル/},
...
window.WATER1_FREQUENCY_SCHEDULER={enabled:true,version:'v9-count-evidence-r7',method:'subject-balanced coverage -> confirmed lower-bound frequency; review -> frequency x personal weakness',profiles:PROFILES.map(function(p){return {theme:p.name,confirmedYearsMin:p.yearsMin,confirmedQuestionsMin:p.questionsMin,weight:p.weight};}),provisional:true,note:'R7 all 60 water-subject questions were topic-TEXT audited from JEMAI official PDFs. Counts remain lower bounds until the 2006-2025 matrix is complete.'};
}
