...
const dup=[];for(let i=0;i<prep.length;i++)for(let j=i+1;j<prep.length;j++){const A=prep[i],B=prep[j];if(A.x.s!==B.x.s)continue;const ts=diceP(A.t,B.t),qs=diceP(A.q,B.q),ps=diceP(A.p,B.p),cs=diceP(A.c,B.c),sameSkill=A.skill===B.skill;const overall=.22*ts+.16*qs+.36*ps+.26*cs+(sameSkill?.04:0);if((overall>=.62&&(ps>=.48||cs>=.55)&&ts>=.28)||(sameSkill&&ts>=.58&&ps>=.58&&cs>=.48))dup.push({a:A.x,b:B.x,skillA:A.skill,skillB:B.skill,ts,qs,ps,cs,overall});}
...
for(const r of dup.slice(0,120))console.log('C3',r.a.id,r.b.id,'score='+r.overall.toFixed(3),'topic='+r.ts.toFixed(3),'stem='+r.qs.toFixed(3),'core='+r.ps.toFixed(3),'answer='+r.cs.toFixed(3),'skill='+r.skillA+'/'+r.skillB,'|',r.a.t,'<>',r.b.t,'|CORE',r.a.p,'<>',r.b.p);
