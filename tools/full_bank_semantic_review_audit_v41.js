...
ok(!h04.o.some(s=>/硝化菌.*Cr\(VI\).*硝酸|クロム酸.*エアストリッピング/.test(s)),'H04 magic distractor regressed');
...
const magic=rewrite.filter(x=>x.tags.includes('MAGIC_DISTRACTOR'));
...
console.log('REVIEWED',review.questions.length,'ACTIVE',bank.length,'ARCHIVED',arc.map(x=>x.id).join(','),'KEEP',counts.KEEP,'STRENGTHEN',counts.STRENGTHEN,'REWRITE',counts.REWRITE,'EXCLUDE',counts.EXCLUDE,'MAGIC_REWRITE',magic.length,'PSEUDO_OR_MIRROR_REWRITE',pseudo.length,'ABSOLUTE_CUE',absolute.length,'STYLE_OR_LENGTH_CUE',style.length);
console.log('BY_SUBJECT',JSON.stringify(bySubject));
console.log('NOTE: REWRITE/STRENGTHEN are explicit quality debt, not a quality-pass claim. This audit passes only when all 200 IDs are semantically reviewed and tracked.');
